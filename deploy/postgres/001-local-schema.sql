...
create table if not exists public.titles (
  id text primary key, slug text not null unique, name text not null, synopsis text,
  media_type text not null check (media_type in ('film','series')),
  release_year integer check (release_year between 1888 and 2100),
  maturity_rating text, runtime_minutes integer check (runtime_minutes is null or runtime_minutes > 0),
  status text not null default 'draft' check (status in ('draft','published','archived')),
  poster_url text, backdrop_url text, logo_url text, accent text, featured boolean not null default false,
  badge text, created_at timestamptz not null default now(), updated_at timestamptz not null default now()
);
...
create table if not exists public.seasons (
  id uuid primary key default gen_random_uuid(), title_id text not null references public.titles(id) on delete cascade,
  season_number integer not null check (season_number > 0), name text, synopsis text,
  created_at timestamptz not null default now(), unique (title_id, season_number)
);

create table if not exists public.episodes (
  id text primary key, season_id uuid not null references public.seasons(id) on delete cascade,
  episode_number integer not null check (episode_number > 0), name text not null, synopsis text,
  runtime_minutes integer check (runtime_minutes is null or runtime_minutes > 0),
  thumbnail_url text, created_at timestamptz not null default now(), unique (season_id, episode_number)
);

create table if not exists public.media_assets (
  id uuid primary key default gen_random_uuid(), title_id text references public.titles(id) on delete cascade,
  ...
  storage_key text, mime_type text, width integer, height integer, duration_seconds integer,
  created_at timestamptz not null default now(), check (title_id is not null or episode_id is not null)
);
...
create table if not exists public.video_sources (
  ...
  source_type text not null check (source_type in ('mp4','hls','dash')), source_url text not null,
  is_default boolean not null default false, width integer, height integer, bitrate_kbps integer,
  created_at timestamptz not null default now()
);
...
create table if not exists public.watch_progress (
  id uuid primary key default gen_random_uuid(), user_id uuid not null references public.local_users(id) on delete cascade,
  title_id text not null references public.titles(id) on delete cascade, episode_id text references public.episodes(id) on delete cascade,
...
create table if not exists public.watchlist (
  ...
  title_id text not null references public.titles(id) on delete cascade,
  created_at timestamptz not null default now(), primary key (user_id, title_id)
);

create table if not exists public.ratings (
  user_id uuid not null references public.local_users(id) on delete cascade,
  title_id text not null references public.titles(id) on delete cascade,
  rating smallint not null check (rating between 1 and 5), created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(), primary key (user_id, title_id)
);

create table if not exists public.subscriptions (
  id uuid primary key default gen_random_uuid(), user_id uuid not null references public.local_users(id) on delete cascade,
  plan text not null, status text not null check (status in ('trialing','active','past_due','cancelled','expired')),
  starts_at timestamptz not null default now(), ends_at timestamptz, provider text,
